...
Google Docs, Google Calendar, and Google Slides REST APIs from Google's Discovery
documents and caches those documents for 24 hours.
...
1. In Google Cloud Console, enable the Google Docs API, Google Calendar API, and
   Google Slides API.
...
and `--page-delay`; `--output` writes the raw response to a file.
...
formats, encrypted keyring storage, service accounts, uploads, or response
sanitization.
